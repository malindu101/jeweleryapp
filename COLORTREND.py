...
# Forecast
st.subheader("🎨 Color Forecast (Selected Month Over 3 Years)")
...
# Select colors to view
selected_colors = st.multiselect("Select Colors to View", list(all_colors), default=list(all_colors))

# Filter for only the selected month and years
selected_years = [year - 2, year - 1, year]
filtered_df = chart_df[
    (chart_df['color'].isin(selected_colors)) &
    (chart_df['date'].dt.month == month) &
    (chart_df['date'].dt.year.isin(selected_years))
]

if filtered_df.empty:
    st.warning("No data found for selected color(s) and month.")
else:
    # 📈 Line Chart
    st.markdown("### 📉 Line Chart (Month: {})".format(month))
    fig, ax = plt.subplots(figsize=(10, 5))
    for color in selected_colors:
        sub_actual = filtered_df[(filtered_df['color'] == color) & (filtered_df['type'] == 'Actual')]
        sub_forecast = filtered_df[(filtered_df['color'] == color) & (filtered_df['type'] == 'Forecast')]
    ...
    ax.set_title(f"{month}/Selected Years Gem Color Usage Trend")
    ax.set_xlabel("Date")
    ax.set_ylabel("Count")
    ax.legend()
    ...
    # 📊 Bar Chart
    st.markdown("### 📊 Bar Chart (Month: {})".format(month))
    ...
    bar_pivot[selected_colors].plot(kind='bar', ax=ax2)
    ax2.set_title(f"Gem Color Usage (Bar Chart) for Month {month}")
    ax2.set_xlabel("Year")
